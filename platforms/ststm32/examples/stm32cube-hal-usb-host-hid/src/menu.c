...
void HID_MenuInit(void)
{
  ...
  BSP_LCD_DisplayStringAtLine(14, (uint8_t *)"Use [Joystick Left/Right]");
  BSP_LCD_DisplayStringAtLine(15, (uint8_t *)"to scroll up/down");
  BSP_LCD_DisplayStringAtLine(16, (uint8_t *)"Use [Joystick Up/Down]");
  BSP_LCD_DisplayStringAtLine(17, (uint8_t *)"to scroll HID menu");
...
{
  /* Handle Menu inputs */
  if(hid_demo.joy_state != state)
  {
    if((state == JOY_UP) && (hid_demo.select > 0))
    {
      hid_demo.select--;
    }
    else if((state == JOY_DOWN) && (hid_demo.select < 2))
    {
      hid_demo.select++;
    }
